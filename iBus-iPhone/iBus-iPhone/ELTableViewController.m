...
@synthesize numberOfRowsInSectionDelegate=_numberOfRowsInSectionDelegate;
...
- (void)loadView{    
    if (nil != self.nibName) {
        ...
        if (CGRectEqualToRect(self.tableViewFrame, CGRectZero)) {
            self.tableViewFrame= self.wantsFullScreenLayout ? CGRectMake(0, 0, 320, 480) : CGRectMake(0, 0, 320, 416);
        }
...
- (NSInteger)tableView:(UITableView *)tableView
 numberOfRowsInSection:(NSInteger)section{
    ...
    if (self.numberOfRowsInSectionDelegate) {
        return self.numberOfRowsInSectionDelegate(tableView, section);
    }else{
